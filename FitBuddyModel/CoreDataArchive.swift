...
public class CoreDataArchive : NSObject {
    ...
    public func exportToDisk (force: Bool) -> NSURL? {
        ...
        let exportPath = CoreDataHelper.localDocsURL().URLByAppendingPathComponent(self.exportFileName)
        CoreDataHelper.migrateDataStore(CoreDataConnection.defaultConnection.theLocalStore, sourceStoreType: CoreDataType.GROUP, destSqliteStore: exportPath, destStoreType: CoreDataType.LOCAL, delete: false)
